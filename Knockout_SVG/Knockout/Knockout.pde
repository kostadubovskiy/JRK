...
void setup() {
  ...
  int numPlayersPTeam = 5;
  pengs = new ArrayList<Penguin>(2 * numPlayersPTeam);
  for(int i = 0; i < numPlayersPTeam * 2; i++) {
   pengs.add(new Penguin(i));
   String file = "Peng.svg";
   if(i % 2 == 0) {
    file = "Blue" + file; 
   } else {
    file = "Red" + file; 
   }
   pengs.get(i).getInd().setPeng(loadShape(file));
  }
...
void draw() {
  if (!_gameOver) {
    if (!_started) { // if not past the start screen
      ...
      for (int i = 0; i < pengs.size(); i++) { // check for bounces
        for (int j = i + 1; j < pengs.size(); j++) {
          pengs.get(i).checkCollision(pengs.get(j));
        }
      }
  ...
  // the great reset
   ...
   for (Platoon t : platoons) {
    for (Penguin p : t.getPlatoon()) { // for all pengs
     if(p.getTarget() != null) {
      p.setVelocity(0.005 * 2.0 * p.getTarget().x, 0.005 * 2.0 * p.getTarget().y); // set velocity to a scalar * displacement vector
